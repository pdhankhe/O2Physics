...
//
// \file jetD0SpectrumSubstructure.cxx
//
...
DECLARE_SOA_TABLE(JetDistanceTable, "AOD", "JETDISTTABLE", jet_distance::JetHfDist, jet_distance::JetPt,
                  jet_distance::JetEta, jet_distance::JetPhi, jet_distance::JetNConst, jet_distance::HfPt,
                  jet_distance::HfEta, jet_distance::HfPhi, jet_distance::HfMass, jet_distance::HfY,
                  jet_distance::HfMlScore0, jet_distance::HfMlScore1, jet_distance::HfMlScore2);
} // namespace o2::aod
struct JetD0Substructure
{
    ...
    HistogramRegistry registry{
        ...
        {{"h_collisions", "event status;event status;entries", {HistType::kTH1F, {{4, 0.0, 4.0}}}},
         ...
         {"h_d0_jet_distance_vs_projection",
    ...
    {
        eventSelectionBits =
            jetderiveddatautilities::initialiseEventSelectionBits(static_cast<std::string>(eventSelections));
    ...
    template <typename T, typename U> void jetCalculateAngularity(T const &jet, U const & /*tracks*/)
    {
        angularity = 0.0;
                                        ...
                                        aod::CandidatesD0Data const &, aod::JetTracks const &tracks)
    {
        ...
        for (const auto &jet : jets)
        {
            ...
            for (const auto &d0Candidate : jet.candidates_as<aod::CandidatesD0Data>())
            {
                ...
                registry.fill(HIST("h_d0_ang"), angularity);

                // filling table
                distJetTable(axisDistance, jet.pt(), jet.eta(), jet.phi(), jet.tracks_as<aod::JetTracks>().size(),
                             d0Candidate.pt(), d0Candidate.eta(), d0Candidate.phi(), d0Candidate.m(), d0Candidate.y(),
                             d0Candidate.mlScores()[0], d0Candidate.mlScores()[1], d0Candidate.mlScores()[2]);
...
{
    return WorkflowSpec{adaptAnalysisTask<JetD0Substructure>(cfgc, TaskName{"jet-d0-substructure"})};
}
